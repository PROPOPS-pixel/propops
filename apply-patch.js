...
// Form note: literal middot (U+00B7), not &middot;
const formNoteOld = '<p class=\"form-note\">14-day free trial \u00b7 $99/mo AUD \u00b7 No payment until day 15</p>';
const formNoteNew = '<p class=\"form-note\">14-day free trial \u00b7 <strong>$69/mo early bird</strong> (locked forever) \u00b7 No payment until day 15</p>';

// That\u2019s = curly apostrophe U+2019
const pricingSubtextOld = '<p style=\"font-size:0.82rem;color:var(--slate-light);margin-bottom:1.5rem;\">That\u2019s $3.30/day \u2014 less than a coffee</p>';
const pricingSubtextNew = '<p style=\"font-size:0.82rem;color:var(--amber-dark);margin-bottom:1.5rem;font-weight:600;\">\u26a1 Locked at $69/mo forever \u2014 goes to $99/mo after June 30</p>';

// FAQ replacement (everything from first FAQ item to the end of the FAQ section, keeping the first 5 items and replacing the last 2 + adding Hugo promo + trades cross-promo)
// Keep: items 1-5 (How fast, Do I need to change, What portals, Is there a contract, Does it work on mobile)
// Replace: items 6-7 (Can I edit, How much) + their closing tags, with Hugo promo + trades cross-promo
...
const changes = [
  ...
  {
    old: pricingSubtextOld,
    new: pricingSubtextNew,
  ...
  },
  {
    old: faqPartialOld,
    new: faqPartialNew,
    label: 'FAQ + Hugo promo + trades cross-promo'
  }
];
...
for (const change of changes) {
  const pattern = change.all ? content.split(change.old).length - 1 : 0;
  if (content.includes(change.old)) {
    if (change.all) {
      let count = 0;
      while (content.includes(change.old)) {
        content = content.replace(change.old, change.new);
        count++;
      }
      console.log('\u2713 ' + change.label + ' (replaced ' + count + 'x)');
    } else {
      content = content.replace(change.old, change.new);
    ...
    console.log('  Looking for: ' + JSON.stringify(change.old.substring(0, 150)));
    failed++;
  }
}
...
console.log('\nVerification:');
console.log('  $69 in stats:', verify.includes('<div class=\"stat-number\">$69</div>'));
console.log('  $69 pricing:', verify.includes('>69</span><span class=\"period\">/mo</span>'));
console.log('  New stripe link:', verify.includes('buy.stripe.com/dRmbJ1bqw89v4Jj0pKdby0a'));
console.log('  Hugo promo:', verify.includes('Ask Hugo instead'));
console.log('  Trades cross-promo:', verify.includes('propops.trade'));
console.log('  FAQ removed:', !verify.includes('How much does it cost?'));